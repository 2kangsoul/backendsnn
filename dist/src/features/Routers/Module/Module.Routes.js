...
const upload = (0, multer_1.default)({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (allowed.includes(file.mimetype)) {
            ...
            cb(new Error("Hanya file PDF atau HTML yang diizinkan"), false);
        }
    },
});
// ── GET /api/modules/:id/sessions/:sessionId/attendances ──────────────────────
// Absensi per sesi (menggantikan /attendances lama)
router.get("/:id/sessions/:sessionId/attendances", verifyToken_1.verifyToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const sessionId = String(req.params.sessionId);
        const attendances = yield prisma_1.default.sessionAttendance.findMany({
            where: { sessionId },
            include: {
                enrollment: {
                    include: {
...
router.get("/:id/sessions", verifyToken_1.verifyToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
router.get("/:id/material/preview", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const moduleId = String(req.params.id);
        const isDownload = req.query.download === "1";
        const module = yield prisma_1.default.module.findUnique({ where: { id: moduleId } });
        if (!(module === null || module === void 0 ? void 0 : module.materialUrl)) {
            return res.status(404).json({ success: false, message: "Materi tidak ditemukan" });
        }
        const response = yield fetch(module.materialUrl);
        if (!response.ok) {
...
router.put("/:id/material", verifyToken_1.verifyToken, upload.single("file"), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!req.file) {
        ...
        const contentType = multerFile.mimetype === "text/html"
            ...
            : "application/pdf";
        const { error } = yield supabaseStorage_1.supabase.storage
        ...
        const { data: publicUrlData } = supabaseStorage_1.supabase.storage
            .from("module-materials")
            .getPublicUrl(fileName);
        const materialUrl = publicUrlData.publicUrl;
        const updatedModule = yield prisma_1.default.module.update({
            where: { id: moduleId },
            data: { materialUrl },
        });
...
router.delete("/:id/material", verifyToken_1.verifyToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const moduleId = String(req.params.id);
        const module = yield prisma_1.default.module.findUnique({ where: { id: moduleId } });
        if (!module)
        ...
        if (module.materialUrl) {
            ...
            if (fileName) {
                yield supabaseStorage_1.supabase.storage.from("module-materials").remove([fileName]);
            }
        }
        const updatedModule = yield prisma_1.default.module.update({
            where: { id: moduleId },
...
router.put("/:id", verifyToken_1.verifyToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const moduleId = String(req.params.id);
        const { status, title, description, location, scheduleDate, durationHour, maxParticipant, notes, latitude, longitude, recurringDay, recurringTime, totalSessions, } = req.body;
        const updateData = {};
        if (status !== undefined)
        ...
        if (durationHour !== undefined)
            updateData.durationHour = durationHour;
        ...
        if (recurringTime !== undefined)
            updateData.recurringTime = recurringTime;
        if (totalSessions !== undefined)
            updateData.totalSessions = Number(totalSessions);
        const updatedModule = yield prisma_1.default.module.update({
            where: { id: moduleId },
            data: updateData,
        });
