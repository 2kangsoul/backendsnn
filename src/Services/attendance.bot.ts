...
function isBreakTime(date: Date = new Date()): boolean {
...
function getLiveTimeout(): number {
...
const NINE_ROUTER_KEY =
  process.env.NINE_ROUTER_KEY || "your_nine_router_api_key";
...
const modelCooldown = new Map<string, number>(); // model → timestamp boleh dipakai lagi
...
function getAvailableModel(): string {
  ...
  console.warn(
    `⚠️ [AI] Semua model cooldown, pakai ${earliestModel} (${Math.ceil((earliest - Date.now()) / 1000)}s lagi)`,
  );
  return earliestModel;
}

function setModelCooldown(model: string, seconds = 65) {
  modelCooldown.set(model, Date.now() + seconds * 1000);
  console.log(
    `⏳ [AI] Model ${model} cooldown ${seconds}s — switch ke model lain`,
  );
...
const BOT_OWNER_PHONES_NORMALIZED = BOT_OWNER_PHONES.map((p) =>
  p.replace(/\D/g, "").replace(/^0/, "62"),
...
function isInternalReasoning(text: string): boolean {
  ...
  return (
    normalized.startsWith("kita perlu") ||
...
async function getStudentContext(phone: string): Promise<string> {
  try {
    ...
    for (const enrollment of user.studentProfile.enrollments) {
      ...
      for (const mod of enrollment.program.modules) {
        ...
        const studentAtts = enrollment.sessionAttendances.filter(
        ...
        if (recentSess.length > 0) {
          ...
          for (const s of recentSess) {
            const att = enrollment.sessionAttendances.find(
              (a) => a.session.id === s.id,
            );
            const status = att?.isPresent ? "✅ Hadir" : "❌ Absen";
...
// ── Typing indicator ──────────────────────────────────────────────────────────
async function showTyping(jid: string) {
  const sock = getWASocket();
  if (!sock || !isWAConnected()) return;
  try {
...
async function stopTyping(jid: string) {
  const sock = getWASocket();
  if (!sock || !isWAConnected()) return;
  try {
...
async function markAsRead(jid: string, msgKey: any) {
  const sock = getWASocket();
  if (!sock || !isWAConnected()) return;
  try {
...
// ── Deteksi tipe pesan ────────────────────────────────────────────────────────
function getMessageInput(msg: any): string | null {
  const m = msg.message || {};
  const text = m.conversation?.trim() || m.extendedTextMessage?.text?.trim();
  if (text) return text;
  if (m.stickerMessage) return "[Pengguna mengirim sticker]";
  if (m.imageMessage) {
  ...
  if (m.videoMessage) {
    const cap = m.videoMessage.caption?.trim();
    return cap
      ? `[Pengguna mengirim video dengan caption: ${cap}]`
      : "[Pengguna mengirim video]";
  }
  if (m.audioMessage || m.pttMessage) return "[Pengguna mengirim pesan suara]";
  if (m.documentMessage) return "[Pengguna mengirim dokumen]";
  ...
  if (m.contactMessage) return "[Pengguna mengirim kontak]";
  return null;
}
...
async function callAI(messages: any[]): Promise<string> {
  ...
  for (let attempt = 0; attempt < FREE_MODELS.length; attempt++) {
    ...
    try {
      ...
      const response = await fetch(`${NINE_ROUTER_URL}/chat/completions`, {
        method: "POST",
        headers: {
      ...
      if (response.status === 402 || response.status === 429) {
        ...
        console.log(
          `⏭️ [AI] Model ${model} skip (${response.status}) — coba model lain`,
        );
        continue;
      }
      ...
      if (!response.ok) {
        const errText = await response.text();
        console.error(
          `❌ [AI] Model ${model} error ${response.status}: ${errText}`,
        );
      ...
      try {
        ...
        const lines = rawText
          .split("\n")
          .filter(
            (l: string) => l.startsWith("data: ") && !l.includes("[DONE]"),
          );
      ...
      console.error(`❌ [AI] Model ${model} error:`, e?.message);
      setModelCooldown(model, 10);
      continue;
    }
  }
...
async function askAI(
  ...
  try {
    ...
    const studentCtx = ownerMode ? null : await getStudentContext(phone);
...
// ── Helper: kirim reply ───────────────────────────────────────────────────────
async function sendReply(rawJid: string, from: string, text: string) {
  const sockNow = getWASocket();
  if (sockNow && isWAConnected()) {
    await sockNow.sendMessage(rawJid, { text });
    console.log(`📤 [Bot] Reply terkirim ke ${rawJid}`);
  } else {
    await sendWAMessage(from, text);
  }
}

type AbsensiState = "WAITING_LIVE_LOCATION" | "LIVE_ACTIVE" | "DONE";
...
interface PendingAbsensi {
  ...
  lastLat?: number;
  lastLon?: number;
  lastLocationAt?: number; // timestamp update lokasi terakhir
  checkedInAt?: number; // timestamp check-in berhasil
}

const pendingByPhone = new Map<string, PendingAbsensi>();
...
function isSessionExpired(pending: PendingAbsensi): boolean {
  return Date.now() > pending.scheduleStartAt + SESSION_EXPIRE_MS;
}

async function confirmCheckIn(
  phone: string,
  pending: PendingAbsensi,
  lat: number,
  lon: number,
) {
  ...
  await updateEnrollmentProgress(pending.enrollmentId, pending.sessionId);
...
export function startAttendanceBot() {
  startReminderBot();
  ...
  cron.schedule("0 16 * * *", async () => {
    ...
    for (const [phone, pending] of entries) {
      if (pending.state !== "LIVE_ACTIVE") continue;
      ...
      const lastUpdateInBreak =
        lastUpdate < breakEndToday && now >= breakEndToday;
      ...
      const isStillInRadius =
        pending.lastLat !== undefined &&
      ...
      if (isLiveStillActive && isStillInRadius) {
        ...
        console.log(
          `✅ [Checkout] ${phone} HADIR PENUH — sesi ${pending.sessionId}`,
        );
      } else {
        ...
        const note = !isLiveStillActive
          ...
          : `Keluar radius kelas sebelum jam 16.00 (jarak terakhir: ${Math.round(haversineDistance(pending.moduleLat, pending.moduleLon, pending.lastLat ?? 0, pending.lastLon ?? 0))}m)`;
  ...
  console.log(
    "🔒 [Checkout Bot] Aktif — checkout otomatis setiap hari jam 16:00 WIB",
  );

  const sockInst = getWASocket();
  if (sockInst) {
    sockInst.ev.on("contacts.upsert", (contacts: any[]) => {
      for (const contact of contacts) {
        if (contact.lid && contact.id) {
          const lid = contact.lid.replace(/@.*/, "");
          const phone = contact.id.replace(/@.*/, "");
          if (lid && phone && lid !== phone) lidToPhone.set(lid, phone);
        }
      }
  ...
  onWAMessage(async (msg) => {
    try {
      ...
      const isOwner =
        BOT_OWNER_PHONES.some((owner) => fromVariants.includes(owner)) ||
        BOT_OWNER_PHONES_NORMALIZED.some((owner) =>
          fromVariants.includes(owner),
        );

      console.log(
        `📨 [Bot] from=${from} resolved=${ownerResolvedPhone} isOwner=${isOwner}`,
      );
      ...
      if (isOwner) {
        ...
        if (userInput) {
          ...
          await markAsRead(rawJid, msg.key);
          await showTyping(rawJid);
          const aiReply = await askAI(from, userInput, true);
          await stopTyping(rawJid);
          await sendReply(rawJid, from, aiReply);
        }
        return;
      }
      ...
      if (!pending) {
        ...
        if (!pending) {
          ...
          if (userInput) {
            await markAsRead(rawJid, msg.key);
            await showTyping(rawJid);
            const aiReply = await askAI(from, userInput);
            await stopTyping(rawJid);
            await sendReply(rawJid, from, aiReply);
          }
          return;
      ...
      if (pending.state === "WAITING_LIVE_LOCATION") {
        ...
        console.log(
          `📍 [Bot] Live location ${replyTo}: ${lat},${lon} — jarak ${jarakFormatted}m`,
        );

        if (jarak <= RADIUS_METER) {
          await confirmCheckIn(replyTo, pending, lat, lon);
        } else {
          await sendWAMessage(
            replyTo,
      ...
      if (pending.state === "LIVE_ACTIVE") {
        ...
        if (liveLocMsg) {
          ...
          console.log(
            `📡 [Bot] Live update ${replyTo}: ${lat},${lon} — jarak ${jarakFormatted}m`,
          );

          if (jarak > RADIUS_METER) {
            console.log(
              `⚠️ [Bot] ${replyTo} keluar radius saat sesi berlangsung (${jarakFormatted}m)`,
            );
        ...
        if (userInput && !liveLocMsg) {
          const aiReply = await askAI(replyTo, userInput);
          await sendWAMessage(replyTo, aiReply);
        }
        return;
      }
      ...
      if (pending.state === "DONE") {
        ...
        if (userInput) {
          ...
          await sendWAMessage(
            replyTo,
            `✅ Absensi kamu sudah selesai diproses untuk sesi ini.`,
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (const session of sessions) {
        ...
        if (!session.module.latitude || !session.module.longitude) {
          console.log(
            `⚠️ [Bot] Sesi ${session.id} tidak punya koordinat — skip`,
          );
          continue;
        }

        console.log(
          `📅 [Bot] Sesi ${session.order} modul "${session.module.title}" dimulai`,
        );
        activeSessions.set(session.id, true);

        const enrollments = session.module.program?.enrollments || [];
        for (const enrollment of enrollments) {
          ...
          const pendingData: PendingAbsensi = {
        ...
        setTimeout(() => {
          ...
          for (const [key, val] of pendingByPhone.entries()) {
            if (val.sessionId === session.id) pendingByPhone.delete(key);
          }
...
async function updateEnrollmentProgress(
  ...
  try {
    const sessionData = await prisma.moduleSession.findUnique({
      where: { id: sessionId },
      select: { moduleId: true },
    });
    if (!sessionData) return;

    const totalSessions = await prisma.moduleSession.count({
      where: { moduleId: sessionData.moduleId, isCancelled: false },
    });
    const presentCount = await prisma.sessionAttendance.count({
    ...
    console.log(
      `📊 [Bot] Progress ${enrollmentId}: ${presentCount}/${totalSessions} = ${progress}%`,
    );
  } catch (e) {
